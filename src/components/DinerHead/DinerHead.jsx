  ...
  return (
    <div className={styles.head_container}>
      <div className={styles.like_review}>
        <FontAwesomeIcon
          className={like.find( diner => diner._id === dinerId) ? styles.like_active : styles.like}
          onClick={like.find( diner => diner._id === dinerId) ? () => handleDislike() : () => handleLike()}
          icon={faHeart}
        />
        <FontAwesomeIcon className={styles.feather} icon={faFeather} />
      </div>
      <div className={styles.diner_name}><h1>{diner?.name}</h1></div>
